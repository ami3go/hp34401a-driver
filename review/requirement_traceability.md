...
| RFDS-003 mandatory shared-core dependency | `pyproject.toml`, `requirements.txt` now depend on `scpi-driver-core` (commit-pinned), plugin environment check resolves it | packaging regression; `docs/scpi_driver_core_review.md` | **CLOSED HP34401A-DEV-001** (superseded 2026-09-10: real package, see `HP34401A-DEC-003`) |
| RFDS-003 runtime core-version reporting | driver info/metadata, plugin, evidence environment now resolve `scpi-driver-core` | source/runtime metadata tests | **CLOSED HP34401A-DEV-005** (superseded 2026-09-10: against the real package) |
| RFDS-003 `BaseInstrumentLibrary` inheritance/orchestration | not integrated; `scpi-driver-core` exists but is deliberately framework-independent and does not provide this layer | authoritative RFDS-003-conformant shared-core contract suite required (does not exist anywhere) | **OPEN HP34401A-DEV-004 / release blocker** -- not advanced by the scpi-driver-core migration |
...
| RFDS-004 transport behavior | `SerialRs232Transport`/`VisaGpibTransport`/`FakeTransport` now backed by `scpi_driver_core.transport.{serial,visa}`/`.simulation.scripted`, whose `Transport` protocol matches RFDS-004 SS8 near-verbatim | core tests, RFDS-019 vectors, simulator-backed integration test (`tests/unit/test_scpi_driver_core_integration.py`); HIL PENDING | wire-level behavior now RFDS-004-shaped; **OPEN HP34401A-DEV-002 (narrowed)** -- driver's own protocol-layer boundary is still string-typed, see `docs/scpi_driver_core_review.md` |
